...
Both our own fine-tunes fall into these shapes: `finetune_whisper.py`'s
output is "full", `finetune_whisper_lora.py`'s output is "lora" -- see
--custom-model / --custom-lora below.
...
step of both `finetune_whisper.py` and `finetune_whisper_lora.py`):
    python3 evaluate_baselines.py \\
        ...
        --custom-model /workspace/whisper-small-sinhala/run6-lr3e-5-bs32
    python3 evaluate_baselines.py \\
        --models openai/whisper-small \\
        --custom-lora /workspace/whisper-small-sinhala-lora/run1-lr1e-4-bs32:openai/whisper-small

Logging the summary table to the same W&B project the training run used, so
the fine-tune's training curves and its final test-set score sit side by
side:
    python3 evaluate_baselines.py --wandb-project whisper --run-name run6-lr3e-5-bs32-eval ...
"""
...
def main():
    ...
    parser.add_argument("--custom-model", action="append", default=[],
                         help="local checkpoint dir (e.g. finetune_whisper.py's --output-dir) to evaluate as a "
                              "full model, on top of --models; repeatable")
    parser.add_argument("--custom-lora", action="append", default=[],
                         help="local adapter dir from finetune_whisper_lora.py's --output-dir, as "
                              "'<adapter-dir>:<base-model>' (e.g. "
                              "/workspace/whisper-small-sinhala-lora/run1:openai/whisper-small); repeatable")
    parser.add_argument("--wandb-project", default=None,
                         help="log the summary table (WER/CER per model) to this W&B project; omit to skip logging")
    ...
    for spec_str in args.custom_lora:
        adapter_dir, _, base_model = spec_str.partition(":")
        if not base_model:
            raise SystemExit(f"--custom-lora expects '<adapter-dir>:<base-model>', got: {spec_str!r}")
        MODEL_SPECS[adapter_dir] = {"type": "lora", "base_model": base_model, "processor_repo": base_model}
        if adapter_dir not in args.models:
            args.models = args.models + [adapter_dir]
    ...
    if args.wandb_project and results:
        import wandb

        run = wandb.init(project=args.wandb_project, name=args.run_name, job_type="eval")
        table = wandb.Table(columns=["model", "wer", "cer", "n_samples"])
        for repo_id, r in results.items():
            table.add_data(repo_id, r["wer"], r["cer"], r["n_samples"])
            run.log({f"test/{repo_id}/wer": r["wer"], f"test/{repo_id}/cer": r["cer"]})
        run.log({"test/summary": table})
        run.finish()
        print(f"\nlogged summary to W&B project '{args.wandb_project}'")
