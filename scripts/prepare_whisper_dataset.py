...
class WhisperASRDataset(torch.utils.data.Dataset):
    ...
    Reads via `pyarrow.parquet.read_table(memory_map=True)`, so the OS page
    cache backs the audio bytes instead of a full Python-heap copy -- opening
    even the 14GB `stratified/train.parquet` is cheap; only accessed rows get
    paged in.
    """
