...
`--train-parquet`/`--eval-parquet` accept `gs://bucket/...` URIs directly
(the datasets are staged in a GCS bucket for cloud GPU runs) as well as local
paths -- see `prepare_whisper_dataset.py`.

Usage (from repo root, on a RunPod GPU pod):
    python3 dse-project/scripts/finetune_whisper_lora.py \\
        --train-parquet gs://sinhaspeech-data/final_split_dataset/stratified/train.parquet \\
        --eval-parquet gs://sinhaspeech-data/final_split_dataset/stratified/validation.parquet \\
...
pipeline without a real training run):
    python3 dse-project/scripts/finetune_whisper_lora.py --smoke-test
