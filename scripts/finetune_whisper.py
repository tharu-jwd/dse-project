"""Fine-tune Whisper-small for Sinhala ASR on the `stratified/` split of the
final datasets. Meant to run on a GPU pod (e.g. RunPod) -- tokenization
(audio decode + log-mel feature extraction + text tokenization) happens
on the fly via `WhisperASRDataset` from `prepare_whisper_dataset.py`, inside
the `DataLoader` workers, overlapped with GPU compute. There is no separate
"tokenize first, then train" step: running this script does both, and it
avoids ever precomputing/storing the ~110GB a fully-cached spectrogram set
for the 123,862-row train split would take.
...
`held_out/` is a secondary diagnostic split, not used here -- see
`dse-project/model-development/data/README.md`.

Usage (from repo root, on a RunPod GPU pod):
    python3 dse-project/scripts/finetune_whisper.py \\
        --output-dir /workspace/whisper-small-sinhala \\
        --num-train-epochs 3 \\
        --per-device-train-batch-size 16 \\
        --wandb-project sinhaspeech

Smoke test on CPU (no GPU needed, a handful of steps, sanity-checks the
pipeline without a real training run):
    python3 dse-project/scripts/finetune_whisper.py --smoke-test
"""
...
DEFAULT_TRAIN = "dse-project/model-development/data/final_split_dataset/stratified/train.parquet"
DEFAULT_EVAL = "dse-project/model-development/data/final_split_dataset/stratified/validation.parquet"
...
def main():
    ...
    parser.add_argument("--eval-parquet", default=DEFAULT_EVAL)
    ...
    print(f"Loading train split: {args.train_parquet}")
    train_dataset = WhisperASRDataset(args.train_parquet, processor)
    print(f"Loading eval split: {args.eval_parquet}")
    eval_dataset = WhisperASRDataset(args.eval_parquet, processor)
