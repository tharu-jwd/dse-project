# Error Analysis — Whisper Small Sinhala (5 fine-tuning runs)
...
Each run gets its own top-level folder, named after its entry in `final-scripts/finetune_tracker.csv` (e.g. `run1-lr3e-5-bs32/`), containing two subfolders:
- `error_analysis/` — `clusters.txt` (themed failure groups), `errors_by_severity.csv` (every wrong sample, worst first), `confusions.txt` (top substitution/deletion/insertion words)
- `run_summary/` — `predictions.csv` (raw reference/prediction pairs), `README.md`, `training_curves.png`, `lr_schedule.png`, `wandb_config.json`, `wandb_summary.json`

`comparison_summary.csv` (one row per run) and this file sit at the top level, shared across every run.

## 1. Headline numbers (15,483 test samples per run)

| Run | Type | Wrong samples | Mean WER | exact_match | minor | moderate | major | severe |
|---|---|---|---|---|---|---|---|---|
| **run1-lr3e-5-bs32** | Full fine-tune | 6,128 (39.6%) | **0.158** | 9,355 (60.4%) | 1,850 | 3,174 | 1,075 | 29 |
| **run6-lr3e-5-bs32_amd** | LoRA (AMD hw) | 7,522 (48.6%) | 0.199 | 7,961 (51.4%) | 2,647 | 3,322 | 1,477 | 76 |
| **run3-lr1e-4-r32-lora** | LoRA | 8,910 (57.5%) | 0.250 | 6,573 (42.5%) | 2,817 | 3,920 | 2,068 | 105 |
| **run2-lr3e-5-bs32_lora** | LoRA (best-epoch2) | 13,993 (90.4%) | **0.575** | 1,490 (9.6%) | 1,356 | 4,436 | 7,860 | 341 |
...
**Ranking: run1-lr3e-5-bs32 > run6-lr3e-5-bs32_amd > run3-lr1e-4-r32-lora >> run2-lr3e-5-bs32_lora.** The full fine-tune generalizes best; both merged/adapter LoRA runs trail it by a moderate margin; run2-lr3e-5-bs32_lora is a clear outlier and should not be used as-is (see §4).
...
1. **Conjunct-consonant / ligature clusters** — keyed on n-grams like `්‍ය`, `‍ය`, `්‍ර`, `‍ර` (the yansaya and rakaransaya conjuncts, which Unicode Sinhala renders with a zero-width joiner). e.g. run1-lr3e-5-bs32 cluster 0 (278 samples) and cluster 2 (401 samples) are almost entirely this. Predictions get the base consonant right but drop or restructure the conjunct: `සත්‍යයද` → `සත්‍යය ද`, `මනෝවිද්‍යාව` → `මනෝ විද්‍යාව`.
2. **Word-boundary / compounding clusters** — the single largest cluster in nearly every run (run1-lr3e-5-bs32 cluster 1: 2,749 samples; run6-lr3e-5-bs32_amd cluster 0: 3,146; run2-lr3e-5-bs32_lora cluster 2: 7,819; run3-lr1e-4-r32-lora cluster 3: 3,220). The model splits words that are written joined in the reference, or joins words that are written apart: `තුන්වැදෑරුම්ය` → `තුන් වැදෑරුම් ය`, `කදාවළලු` → `කඳා වළලු`, `ටීවිවල` → `ටීවි වල`.
...
4. **Honorific / religious-register cluster** (`වහන්සේ`, `න්සේ`) — present as its own cluster in run6-lr3e-5-bs32_amd (123), run2-lr3e-5-bs32_lora (259), run3-lr1e-4-r32-lora (151): `භික්‍ෂූන්වහන්සේ` → `භික්ෂූන් වහන්සේ`, `උන්වන්සේගේ` → `උන්වහන්සේගේ`.
5. **Degenerate repetition ("hallucination loop") cluster** — the single worst individual samples in run6-lr3e-5-bs32_amd and run3-lr1e-4-r32-lora (WER up to 7.60) are the decoder getting stuck repeating one word/phrase dozens of times (`මෙම මෙම මෙම...` repeated ~40 times for a 9-word reference). Not present as a distinct cluster in run1-lr3e-5-bs32.
...
Sinhala conjuncts (යන්සය, රකාරාංශය) can be encoded in Unicode with or without an explicit ZWJ, and different tools/keyboards normalize them differently. The recurring `්‍ය` / `‍ය` / `්‍ර` / `‍ර` clusters in run1-lr3e-5-bs32 (679 of 6,128 wrong samples, ~11%) point to the training transcripts not being normalized to one encoding, so the model learned an inconsistent mapping for these characters specifically. **Recommendation: run a Unicode NFC/ZWJ-normalization pass over all transcripts before the next training run** — this is a cheap, one-time data fix that should directly reduce this cluster.
...
The exact-repeated-token failure mode (`මෙම මෙම මෙම...`) only shows up prominently in **LoRA** runs (run6-lr3e-5-bs32_amd, run3-lr1e-4-r32-lora), not the full fine-tune (run1-lr3e-5-bs32). This is a classic symptom of a decoder that hasn't been adapted enough to confidently terminate generation on out-of-distribution or acoustically ambiguous input (background noise, very short utterances, rare words) — it falls back to repeating its highest-probability token instead of stopping. A full fine-tune updates all weights and is more robust to this; a low-rank adapter has less capacity to correct the base model's decoding behavior on hard inputs. **Recommendation: add a repetition penalty / no_repeat_ngram_size at generation time for the LoRA checkpoints**, and consider a higher LoRA rank if this persists after that.

### f) run2-lr3e-5-bs32_lora is a clear outlier — likely an undertrained/wrong checkpoint, not a data problem
run2-lr3e-5-bs32_lora's error profile isn't just "a bit worse" — 90% of samples are wrong (vs. ~40–58% for the others) and the *shape* of its errors is different: its largest cluster (7,819 samples, avg WER 0.64) has no coherent linguistic theme, and its confusions include basic word confusions the other runs don't make (`මොකද`→`මුකද`, `විශාල`→`විෂාල`, `එයින්`→`එහෙන්`) alongside the same orthographic issues seen elsewhere. Since this run is labeled "best-epoch2", it suggests either:
  ...
  **Recommendation: re-check run2-lr3e-5-bs32_lora's training logs/config before drawing any conclusions from it, and re-run evaluation on its later checkpoints (`checkpoint-3871`, `checkpoint-7742`) if they exist** — `best-epoch2` may simply have been selected on too little training.

## 4. Practical takeaways

- **Use run1-lr3e-5-bs32 (full fine-tune)** as the primary model; it has the lowest error rate and the cleanest error profile.
- **Don't ship run2-lr3e-5-bs32_lora as-is** — its errors aren't representative of the LoRA approach's ceiling, they're representative of an undertrained/misconfigured run.
