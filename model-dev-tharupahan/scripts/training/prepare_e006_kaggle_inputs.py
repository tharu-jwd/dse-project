#!/usr/bin/env python3
"""Stage the private 82,835-row (100-hour) E006 Sinhala scale dataset for Kaggle."""
...
SHARDS = ROOT / "reports/e006-preparation/sinhala-shards"
OUTPUT = ROOT / "reports/kaggle/e006-input-dataset"
...
def main() -> None:
    ...
    if shard_manifest["rows"] != EXPECTED_ROWS:
        raise SystemExit("E006 shard row count mismatch")
    if shard_manifest["shards_combined_sha256"] != EXPECTED_SHARDS_COMBINED_SHA256:
        raise SystemExit("E006 shard combined-hash mismatch")
    ...
    metadata = {
        "title": "Sinhala ASR E006 Private 100h Inputs",
        "id": "tharupahan/sinhala-asr-e006-inputs",
        "licenses": [{"name": "other"}],
        "description": (
            "Private 82,835-row deterministic v4 Sinhala-source training subset "
            "(100.0003 hours, superset of the frozen E005 50,000-row selection) "
