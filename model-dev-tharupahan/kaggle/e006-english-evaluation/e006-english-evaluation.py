"""Evaluate E006 on the frozen LibriSpeech test-clean benchmark."""
...
OUTPUT = WORK / "e006-english-evaluation"
MODEL = WORK / "whisper-small"
ADAPTER_SHA256 = "00a43073efe812c4aeae76f817b0e73f7ac56357f4e2175b6e8f05615ccbe711"
...
def main() -> None:
    ...
    result_path = one_file("e006-kaggle-training-result.json")
    training_result = json.loads(result_path.read_text())
    if training_result["adapter_sha256"] != ADAPTER_SHA256:
        raise RuntimeError("E006 training result adapter hash mismatch")
    adapter = result_path.parent / "e006-training/final-adapter"
    if sha256(adapter / "adapter_model.safetensors") != ADAPTER_SHA256:
        raise RuntimeError("attached E006 adapter hash mismatch")
    subprocess.run(
    ...
    subprocess.run(
        [
            ...
            str(adapter),
            "--variant",
            # The frozen evaluator snapshot predates the E004/E005/E006
            # display labels. Run its identical adapter path under the
            # supported E003 label, then correct only provenance fields
            # and filenames below -- prediction content is never
            # transformed.
            "e003",
        ],
        check=True,
    ...
    (OUTPUT / "e006-runtime.json").write_text(
    ...
    (WORK / "e006-kaggle-english-evaluation-result.json").write_text(
