"""Evaluate E005 on the frozen LibriSpeech test-clean benchmark."""
...
OUTPUT = WORK / "e005-english-evaluation"
MODEL = WORK / "whisper-small"
ADAPTER_SHA256 = "cc4930df9d1207eee4e008db1e47f19dd8dc11763685696686fadeba3a5f42f3"
...
def main() -> None:
    ...
    result_path = one_file("e005-kaggle-training-result.json")
    training_result = json.loads(result_path.read_text())
    if training_result["adapter_sha256"] != ADAPTER_SHA256:
        raise RuntimeError("E005 training result adapter hash mismatch")
    adapter = result_path.parent / "e005-training/final-adapter"
    if sha256(adapter / "adapter_model.safetensors") != ADAPTER_SHA256:
        raise RuntimeError("attached E005 adapter hash mismatch")
    subprocess.run(
    ...
    subprocess.run(
        [
            ...
            str(adapter),
            "--variant",
            # The frozen evaluator snapshot predates the E004/E005 display
            # labels. Run its identical adapter path under the supported
            # E003 label, then correct only provenance fields and filenames
            # below -- prediction content is never transformed.
            "e003",
        ],
        check=True,
        env=environment,
    ...
    (OUTPUT / "e005-runtime.json").write_text(
    ...
    (WORK / "e005-kaggle-english-evaluation-result.json").write_text(
