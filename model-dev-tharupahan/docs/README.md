# Documentation Index

## Project

- [Plan](project/plan.md)
- [Compute plan](project/compute-plan.md)

## Data

- [Dataset operations and versions](data/dataset.md)
- [Audio audit](data/audio-audit.md)
- [Transcript policy](data/text-policy.md)
- [Review provenance](data/review-provenance.md)

## Model development

- [Training operations](training/training.md)
- [Evaluation protocol](evaluation/evaluation.md)
- [Experiment registry](experiments/README.md)

## Audits and historical evidence

- [Historical team-work audit](audits/historical-audit.md)
- [Label-refinement A/B](audits/label-refinement-ab.md)
- [Refinement-model benchmark](audits/refinement-model-benchmark.md)

Stable policies and operational guides live in their subject directories.
Individual run results live under `experiments/`; generated machine-readable
artifacts remain under `reports/` and are not duplicated here.
