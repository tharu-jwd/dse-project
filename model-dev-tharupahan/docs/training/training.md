...
E002 completed and established the first meaningful Sinhala improvement, but
failed the subsequent frozen English-retention evaluation. Its artifacts remain
the historical control for later comparisons.
...
LibriSpeech replay was prepared on a CPU-only disposable runtime and transferred
in four hash-verified shards. E003 passed its checkpoint-resume smoke and
improved Sinhala, but raw uppercase reference replay still failed English
retention.

## E004 and E005

E004 replaced the raw English references with outputs generated by the
untouched Whisper-small checkpoint on the same replay audio. It preserved the
E003 Sinhala result and passed the frozen English gate, so teacher-target replay
is the current retention method.

E005 changes only the Sinhala data scale and exposure: 50,000 deterministic v4
source rows (60.31 hours) plus 5,556 teacher-target replay occurrences, trained
