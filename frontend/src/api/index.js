...
function token() {
  return localStorage.getItem('sinhaspeech_token')
}
...
export const api = {
  async login(email, password) {
    ...
    if (!user)
      throw Object.assign(new Error('The email or password is incorrect.'), {
  ...
  async getJob(jobId) {
    ...
    if (job.polls >= 3) {
      ...
      if (job.status === 'COMPLETED' && !transcripts.some((item) => item.id === job.transcript.id))
        transcripts.unshift(job.transcript)
    }
    return {
      jobId,
      status: job.status,
      transcriptId: job.status === 'COMPLETED' ? job.transcript.id : undefined,
      message:
        job.status === 'FAILED'
  ...
  async getTranscripts() {
    ...
    return clone(
      user?.role === 'TEACHER'
        ? transcripts.filter((t) => t.ownerId === user.id || t.type === 'LECTURE')
        : transcripts.filter((t) => t.ownerId === user?.id),
    )
  },
  ...
  async updateTranscript(id, updates) {
    if (!USE_MOCK_API)
      return request(`/transcripts/${id}`, { method: 'PATCH', body: JSON.stringify(updates) })
    await pause(500)
  ...
  async getQuizzes({ teacher = false } = {}) {
    ...
    await pause()
    return clone(teacher ? quizzes : quizzes.filter((q) => q.status === 'PUBLISHED'))
  },
  async getQuiz(id) {
  ...
  async saveQuiz(quiz) {
    ...
    if (!USE_MOCK_API)
      return request(quiz.id ? `/quizzes/${quiz.id}` : '/quizzes', {
        method: quiz.id ? 'PATCH' : 'POST',
        body: JSON.stringify(payload),
      })
    await pause(500)
    const index = quizzes.findIndex((q) => q.id === payload.id)
    if (index >= 0) quizzes[index] = clone(payload)
    else quizzes.unshift(clone(payload))
    return clone(payload)
  },
  async publishQuiz(id) {
    if (!USE_MOCK_API) return request(`/quizzes/${id}/publish`, { method: 'POST' })
    const quiz = quizzes.find((q) => q.id === id)
    return api.saveQuiz({ ...quiz, status: 'PUBLISHED' })
  },
  async submitQuiz(id, answers) {
    if (!USE_MOCK_API)
      return request(`/quizzes/${id}/submit`, { method: 'POST', body: JSON.stringify({ answers }) })
    await pause(600)
    const quiz = quizzes.find((q) => q.id === id)
  ...
  async getSubmission(id) {
    ...
    await pause(300)
    const result = submissions.find((s) => s.id === id)
    if (!result) throw new Error('Submission was not found.')
    return clone(result)
  },
  async reviewSubmission(id, review) {
    if (!USE_MOCK_API)
      return request(`/submissions/${id}/review`, { method: 'PATCH', body: JSON.stringify(review) })
    await pause(500)
